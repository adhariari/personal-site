	...
	/*--------------------------------------------------------------------------*/
		...
		if (empty($lokasi_file)){
			...
			if (trim($hp)=="" or ! is_numeric(trim($hp))) {
				...
				mysqli_query($connect, "UPDATE pengguna SET passwd_origin = '$pass',
										nm_lengkap = '$nama_user',
										email_pengguna = '$email',
										hp_pengguna = '$hp',
										status_pengguna = '$_POST[status_user]',
										blokir = '$_POST[blokir]',
									WHERE token_pengguna = '$_POST[tid]'");
				...
				if(in_array($file_ext,$expensions)== false){
					...
					$data_gbr = mysqli_query($connect, "SELECT img_pengguna FROM pengguna WHERE token_pengguna = '$_POST[tid]'");
