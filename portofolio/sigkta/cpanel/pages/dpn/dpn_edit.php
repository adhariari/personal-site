  ...
  //Simpan gambar dalam ukuran sebenarnya
		...
		if (empty($lokasi_file)){
			...
			if (! is_numeric(trim($longitude)) OR ! is_numeric(trim($latitude))) {
				...
				$cek = mysqli_query($connect, "UPDATE dpn SET kecamatan = '$kecamatan',
					desa = '$desa',
					blok = '$blok',
					unit = '$unit',
					kawasan = '$kawasan',
					longitude = '$longitude',
					latitude = '$latitude'
					WHERE id_dpn = '$_POST[id]'");
				if($cek){
					echo "<script>alert('Data DPN sudah di update.');</script>";
			...
			if (! is_numeric(trim($longitude)) OR ! is_numeric(trim($latitude))) {
				...
				if(in_array($file_ext,$expensions)== false){
					...
					$data_gbr = mysqli_query($connect, "SELECT gambar FROM dpn WHERE id_dpn = '$_POST[id]'");
					$r    	= mysqli_fetch_array($data_gbr);
					@unlink('gambar/img_dpn/'.$r['gambar']);
